...
class QueueElement(object):
    def __init__(self, pkt=None, sender_eth_addr=None, interface_to_send=None):
        self.pkt = pkt
        self.pkt[Ethernet].src = sender_eth_addr
        self.sender_eth_addr = sender_eth_addr
        self.interface_to_send = interface_to_send


class QueueEntry(object):
    """__init__() functions as the class constructor"""
    def __init__(self, pkt=None, sender_eth_addr=None, target_ip_addr = None, interface_to_send=None, push_time=None, retry=0):
        newEle = QueueElement(pkt, sender_eth_addr, interface_to_send)
        #eles contains many elements, each of the element is the class above. because all the elements in the same entry shares rety_time, push_time and target_ip_addr
        #while the interface_to_send and sender_eth_addr are private
        self.eles = [newEle]
...
class Router(object):
    def __init__(self, net):
        ...
        self.arp_table = []
        self.forward_table =[]
        self.pkt_queue = {} #key is target ip addr, value is queue_entry
    ...
    def look_up(self, dest):
        ...
        for entry in self.forward_table:
            ...
            if intf.ipaddr == dest_addr:                           # ourselves
                return -2, entry.interfaceName, dest #target is me
        ...
        if next_hop == None:
            return -1, next_intf, next_hop #not found
        else:
            return 0, next_intf, next_hop #found it 
    ...
    def check_queue(self):
        ...
        for key in list(self.pkt_queue): #copy the key to avoid runtime error
            entry = self.pkt_queue[key]
            if time.time() - entry.push_time - 1 > err: #need to retry
                if entry.retry < RETRY_TIME: # not timeout
                    entry.push_time = time.time()
                    entry.retry += 1
                    self.send_arp_request(entry.eles[0].interface_to_send, entry.target_ip_addr)
                else:                       #this entry is going to timeout
                    for ele in entry.eles:  #for each packet in this entry
                        pkt = ele.pkt
                        self.report_error(pkt, 2)
                    need_to_remove.append(key) #remove this entry later
        #delete the timeout entry after the loop
        for key in need_to_remove:
            del self.pkt_queue[key]
    ...
    def check_response_and_send(self, arp):
        ...
        entry = self.pkt_queue[arp.senderprotoaddr]
        for ele in entry.eles:
            pkt = ele.pkt
            pkt[Ethernet].dst = arp.senderhwaddr
            interface_to_send = ele.interface_to_send
            self.net.send_packet(interface_to_send, pkt)

        #delete the entry because we have sent them out
        del self.pkt_queue[arp.senderprotoaddr]


    #push pkts in the queue, let them wait for the response of arp
    def prepare_pkt(self, pkt, next_intf, nextHop):

        next_intf_ethaddr = self.net.interface_by_name(next_intf).ethaddr
        if nextHop not in self.pkt_queue:
            self.send_arp_request(next_intf, nextHop) #get nextHop ethernet address
            newEntry = QueueEntry(pkt, next_intf_ethaddr, nextHop, next_intf, time.time(), 1)
            self.pkt_queue[nextHop] = newEntry

        else:
            newEle = QueueElement(pkt, next_intf_ethaddr, next_intf)
            self.pkt_queue[nextHop].eles.append(newEle)
    ...
    def report_error(self, pkt, error_code):
        ...
        if nextHop in self.known_arp_ipaddr:
            ...
            self.prepare_pkt(pkt, intf_name, nextHop)
    ...
    def router_main(self):    
        ...
        while True:
                ...
                if arp is not None:
                    ...
                    if succeed ==  -1:   #not found a match, drop
                        ...
                        if nextHop in known_arp_ipaddr:
                            ...
                            self.prepare_pkt(pkt, next_intf, nextHop)
                        ...
                        if icmp is not None and icmp.icmptype == ICMPType.EchoRequest:
                            ...
                            if icmp.icmptype == ICMPType.EchoRequest:   # is echo request
                                ...
                                if nextHop in known_arp_ipaddr:
                                    ...
                                    self.prepare_pkt(icmp_reply, next_intf, nextHop)
                        else:
            ...
            self.check_queue()  #check to resend pkt periodically
